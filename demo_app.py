...
def health_bars(row):
    hs, ss = row["health_score"], row["sustainability_score"]
    fig = go.Figure()
    for label, val, col, i in [
        ("Health Score", hs, score_col(hs), 0),
        ("Sustainability", ss, score_col(ss), 1),
    ]:
        fig.add_trace(go.Bar(
            ...
            text=[f"{val}/100"],
            textposition="outside",
            textfont=dict(size=13, color=col, family="Segoe UI"),
            width=0.4,
            hovertemplate=f"{label}: {val}/100<extra></extra>",
        ))
        # Background bar (max)
        fig.add_trace(go.Bar(
            name="",
...
with c3:
    ...
    hc  = C_AMBER   # Health Score — always amber/orange
    sc2 = C_GREEN   # Sustainability Score — always green
    hs  = row["health_score"]
